...
fs.readFileSync(process.argv[2]).toString().split('\n').forEach(function (line) {
    if (line !== "") {
        var self_describing = true;
        var digits = (""+line).split("");
        for(var position=0;position<digits.length;position++)
            {
                var number_on_position = digits[position];
                //console.log(number_on_position);
                //console.log((""+line).split(position).length-1);
                if (number_on_position!=((""+line).split(position).length-1))
                    {
